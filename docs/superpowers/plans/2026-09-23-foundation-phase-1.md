...
1. **`--affected` resolves its base differently per event, and fails on pull requests.** On `push`, turbo reads `github.event.before` and scoping works. On `pull_request` it tries to resolve the bare ref `main`, which `actions/checkout` never creates locally — it checks out the merge ref in detached HEAD — so resolution fails _regardless of `fetch-depth`_ and turbo falls back to "assume all files changed". **Corrected 2026-09-23 from live CI logs:** `fetch-depth: 0` is **not** what makes `--affected` work. It is load-bearing for `audit-authors.sh`'s `origin/main..HEAD` range, which is a real and separate reason to keep it. `TURBO_SCM_BASE` is what actually fixes scoping, and the fallback is only _currently_ fail-safe — on a shallow clone turbo resolves a base, finds no history, runs zero tasks and exits 0 silently.
2. **A required status check that has never reported green _on a pull request_ blocks every PR.** The spec calls this out in §13.1, but understates it: green-on-push is not enough. `gitleaks-action` passed on `push` and hard-failed on the first `pull_request`, because the two events take different code paths with different tokens. Task 16 therefore requires **only** `authors` and `secrets` at first-push time, and promotes `check` only after it has reported green on a real PR.
3. **macOS is case-insensitive; the CI runner is not.** A relative documentation link written as `north-star.md` but stored as `North-Star.md` resolves locally and 404s in CI. `check-docs.sh` (Task 14) compares each link target against the exact name `git ls-files` reports, not against the filesystem's answer.
4. **`.dockerignore` and `.liaraignore` drift apart silently.** §9.3 requires them byte-identical because Liara reads exactly one ignore file. Nothing would notice until a deploy shipped `node_modules`. `check-docs.sh` (Task 14) diffs them and fails if they differ.
