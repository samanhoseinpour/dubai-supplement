...
**`*.tsbuildinfo` is deliberately not a turbo `build` output.** It is build _state_, not an artifact anything consumes, and it is the one file whose meaning — "this project is already up to date" — depends on mtimes turbo does not model. Caching it lets a restored cache hand back a "do not rebuild" token keyed to a source hash it need not correspond to: `tsc -b` decides by mtime, so a source file restored from a backup rewinds time, the build is skipped, and the stale `dist` is then cached against the **new** hash. That poisoned entry survives later runs and turns `pnpm check` green over code nobody compiled. It was observed once during Phase 2. Rebuilding all three packages from cold costs 2.6 s, which is the whole price of removing it. `dist/**` stays an output.

Note `turbo.json` is parsed as **strict JSON** by `apps/api/test/boundaries.test.ts`, so it carries no comments — which is why this note lives here.
