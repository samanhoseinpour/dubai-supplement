...
5. **`exactOptionalPropertyTypes` meets an optional column.** `base.json` sets it. Drizzle's insert type for a nullable column accepts `string | null` but not `string | undefined`, so the natural `{ description: input.description }` for an absent optional fails to compile — and the natural fix (`?? null`) changes what is stored. Task 13 pins the intended mapping: `outbox_events.last_error` is the first nullable column in the codebase, and its test inserts a row with the field absent and reads back `null` rather than the string `"undefined"`.
