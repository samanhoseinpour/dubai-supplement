...
**Recorded for 3c:** `publicApi` reads `API_INTERNAL_URL` itself on first use, so `apps/web/lib/env.ts`'s `getServerEnv()` still has no caller — 3c decides whether it stays for `requestApi()` or goes; the `e2e` job gains Postgres, Redis, the API server and `pnpm db:seed` after `pnpm db:migrate`; the brand tile's body is the seed's `description`, which Saman may reword in `seed-brands.ts` — a change there is data, not a migration; the storefront's `getBrand` validates the slug with `slug` from `@ds/contracts` and calls `notFound()` without an API call (today a malformed slug such as `/brands/MuscleTech` would be a 400 that 3c's design rethrows into `error.tsx`); the API throttle (120 requests a minute per endpoint per source IP) is shared by the whole storefront, because every `publicApi` call leaves the web container and `publicApi` runs inside `'use cache'`, so it cannot forward `X-Forwarded-For` — the 3c spec must budget `sitemap.ts`'s uncached list call and random-slug cache misses; network failures reach callers as a bare `TypeError: fetch failed`, not an `ApiError`, and there is no request timeout (consider `AbortSignal.timeout` through the client's `fetch` option); `apps/web` gets a `// @ts-expect-error` on an unknown route through the built `@ds/api-client`, the only place a regression of `paths` to `any` can be caught (Deviation 12).

**Follow-ups from the whole-branch review (2026-09-26):**

- **Before the products slice copies the module:** one presenter in `api/` for both routes, with the service returning entities — today `list` maps to the HTTP shape in the service, the detail route in the controller.
- **Products slice** (the first cross-module foreign key): `export * as tables` should be importable only from another module's `infrastructure/schema.ts`.
- **Search slice:** `pg_trgm` needs a non-`C` `datctype` — the check is in `docs/runbooks/first-deploy.md` §1 now.
- **Before a second kind of client (the admin app):** error responses are documented as `application/json` in `openapi.json` although the server sends `application/problem+json`; Fastify writes `FST_ERR_BAD_URL` (bad percent-encoding) and the 431 for an over-16-KiB request line itself, outside `ProblemFilter`.
- **Before the first deploy:** `docs/runbooks/first-deploy.md` §2 (and `go-live.md` §5's variable list) says `TRUST_PROXY=1`, which spec §5.5 says fails closed.
- **Hardening:** dependency-cruiser reports workspace packages and `uuid` as `couldNotResolve`, so `domain-is-pure` cannot see them (add `enhancedResolveOptions` or match bare names); the types-only `@ds/api-client → @ds/contracts` edge is unenforced (`no-restricted-imports` with `allowTypeImports`).
- **Tests, when their module is next touched:** a code-point cap test with an astral character; a rehydrate test that proves no re-normalisation (an Arabic ك prop stays ك); a 64-character slug accepted; the `description = ''` CHECK at the table; the logger-spy test in `brand-created.logger.test.ts` restoring in `finally`; `server.test.ts` stubbing `fetch` and asserting the request URL; a malformed `API_INTERNAL_URL` error naming the variable.
- **Nits:** the `brands_slug_key` literal is written twice (`schema.ts` and the repository); lefthook's ESLint step warns "File ignored" on generated files (`--no-warn-ignored`); `pnpm db:seed` shows only turbo's summary — the seed's lines are in `apps/api/.turbo/turbo-seed.log`.
- **Revisit on an upstream release:** `openapi-typescript` 7.13.0 declares a TypeScript `^5` peer and runs on 6.0.3.
