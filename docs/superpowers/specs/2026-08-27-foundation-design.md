...
- **Ecosystem timing (re-verified 2026-09-23):** every claim below was re-checked against the registry and upstream trackers after a four-week pause.
  ...
  - **NestJS moves to 12.1.0.** Not cosmetic: `nestjs-pino` 5.2.0 — the first release supporting Nest 12 — declares a peer floor of `@nestjs/core ^12.0.2`.
...
| Backend | NestJS 12.1.x, `@nestjs/platform-fastify` 12.1.x, Fastify 5.12.x, `@nestjs/cli` 12.0.5 | ESM (`"type": "module"`, `nodenext`); SWC builder with `typeCheck: true`; fallback NestJS 11.2.x CJS if the spike fails (§5.1) |
...
- shadcn 4.21 initialized with `pnpm dlx shadcn@<pinned> init --rtl -b base` → `components.json` `rtl: true`, Base UI primitives, `DirectionProvider` in the root layout, `dir="rtl"` passed explicitly to portal components (tw-animate-css RTL bug).
...
- Playwright (`apps/web/playwright.config.ts`, `baseURL: http://localhost:3000`) starts both servers itself: `webServer: [{ command: 'pnpm --filter api start', url: 'http://localhost:3001/health/live', env: { PORT: '3001', PROCESS_ROLE: 'all' } }, { command: 'pnpm --filter web start', url: 'http://localhost:3000/health' }]`, `reuseExistingServer: !process.env.CI`; `locale: 'fa-IR'`, `timezoneId: 'Asia/Tehran'`, one desktop and one mobile project. Assertions: `new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa']).analyze()` on `/`, `/brands`, `/brands/muscletech` with `expect(results.violations).toEqual([])`; `html[dir="rtl"][lang="fa"]` exists; `page.evaluate(async () => { await document.fonts.ready; return [...document.fonts].some(f => /vazirmatn/i.test(f.family) && f.status === 'loaded'); })` is `true`; `/brands` lists exactly the six seeded names in the API's order; `/brands/muscletech` contains «ماسل‌تک» (with U+200C); `/brands/no-such-brand` contains the `not-found.tsx` heading and `meta[name="robots"][content*="noindex"]`. Runs in CI against the real API and database (§10).
...
9. Approve the two `pnpm dlx` scaffold commands when the plan reaches them (`@nestjs/cli … new api`, answering ESM; `shadcn … init --rtl -b base`) — they are *ask* rules for Claude (§12.2).
...
| `@nestjs/*` (core, common, platform-fastify, testing) | 12.0.1 | **12.1.0** | Forced by nestjs-pino 5.2.0's peer floor `^12.0.2`. Also adds array global prefixes, built-in cookies/CSRF/security headers and Fastify multipart — all **unused**, the spec keeps `@fastify/helmet` + `@fastify/cookie` (ADR) |
...
| `@nestjs/cli` | 12.0.0 | 12.0.5 | Fixes ESM emit under SWC and tsconfig-relative path aliases — directly relevant here |
| `@nestjs/swagger`, `@nestjs/config` | 12.0.0 | 12.0.2 / 12.0.1 | `standardSchemaConverter` byte-identical |
...
**Canonical local values** (identical in `apps/*/.env.example`, `infra/compose.yaml`, the CI `services:` blocks and the Testcontainers setup): api — `NODE_ENV=development`, `PROCESS_ROLE=all`, `PORT=3001`, `LOG_LEVEL=debug`, `DATABASE_URL=postgres://dubaisupp:dubaisupp@127.0.0.1:5432/dubaisupp`, `DATABASE_POOL_MAX=10`, `REDIS_URL=redis://127.0.0.1:6379/0`, `S3_ENDPOINT=http://127.0.0.1:9000`, `S3_REGION=default`, `S3_BUCKET=dubaisupp`, `S3_ACCESS_KEY_ID=rustfsadmin`, `S3_SECRET_ACCESS_KEY=rustfsadmin`, `S3_FORCE_PATH_STYLE=true`, `CORS_ORIGINS=` (empty), `TRUST_PROXY=1`, `OUTBOX_POLL_MS=1000`, `OPENAPI_UI_ENABLED=true`; web — `NODE_ENV=development`, `PORT=3000`, `API_INTERNAL_URL=http://localhost:3001`, `NEXT_PUBLIC_SITE_URL=http://localhost:3000`. **Required (no default):** `DATABASE_URL`, `REDIS_URL`, `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`; every other key's Zod default equals its `.env.example` value, so `test:integration` boots with only what the Testcontainers setup writes.
